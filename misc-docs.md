---
title: Misc Docs
feature_text: |
  ## Misc Docs
  Posters, talks, reports, and other documents
feature_image: "https://picsum.photos/1300/400?image=1067"
excerpt: "A collection of presentations, term papers, reports, and posters."
aside: true
---

This page is a place to collect PDFs, presentations, posters, and reports. Files can live under `assets/docs/`.

## Presentations And Posters

- Exploring the Tearing Mode Instability in 3D using Direct Numerical MHD Simulations.  
  Poster presentation.

- Whistler Waves.  
  Presentation and term paper.

- Homogeneous Nucleation.  
  Presentation.

## Reports And Term Papers

- Magnetohydrodynamics and the Tearing Mode Instability.  
  Project report.

- H-alpha Line and its Significance in Astrophysics.  
  Term paper.

- Simulation and Stability of Lagrange Point L1.  
  Report.

- Torque-dependent Remodeling of the Bacterial Flagellar Motor.  
  Paper presentation.
